...
"""Module containing fused attention forward and backward pass.
   From jax.experimental.pallas.ops.attention, modified for cross attention.
   Currently for JAX version 0.4.28.
"""
...
def mha_forward_kernel(
  ...
  def body(start_k, carry):
    ...
    if causal or q_segment_ids_ref is not None:
      mask = None
...
def mha(
  ...
  in_specs = [
      pl.BlockSpec(
          lambda _, j, k: (j, 0, k, 0), (None, seq_len_q, None, head_dim) # q
      ),
      pl.BlockSpec(
          lambda _, j, k: (j, 0, k, 0), (None, seq_len_k, None, head_dim) # k
      ),
      pl.BlockSpec(
          lambda _, j, k: (j, 0, k, 0), (None, seq_len_k, None, head_dim) # v
      ),
  ]
  in_specs.append(
      None  # type: ignore[arg-type]
      if segment_ids is None
      else pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len_q)) # mq
  )
  in_specs.append(
      None  # type: ignore[arg-type]
      if segment_ids is None
      else pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len_k)) # mk
  )
  ...
  if attention_type == "self":
    ...
    q_segment_ids = segment_ids[0]
    kv_segment_ids = segment_ids[1]
...
def _mha_forward(
  ...
  in_specs.append(
      None  # type: ignore[arg-type]
      if segment_ids is None
      else pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len_q))
  )
  in_specs.append(
      None  # type: ignore[arg-type]
      if segment_ids is None
      else pl.BlockSpec(lambda _, j, k: (j, 0), (None, seq_len_k))
  )
  assert attention_type in ("self", "cross")
  if attention_type == "self":
    q_segment_ids = segment_ids
...
def mha_backward_kernel(
  ...
  def outer_loop(start_k, _):
    ...
    kv_segment_ids = (
        None
        if kv_segment_ids_ref is None
        else pl.load(kv_segment_ids_ref, (pl.ds(start_k * block_k, block_k),))
    )
    ...
    def inner_loop(start_q, carry):
      ...
      q_segment_ids = (
          None
          if q_segment_ids_ref is None
          else pl.load(q_segment_ids_ref, (pl.ds(start_q * block_q, block_q),))
      )
...
def _mha_backward(sm_scale: float, causal: bool, attention_type: str, block_q: int, block_k: int,
  ...
  if backward_pass_impl == "xla":
    ...
    in_specs = [
        pl.BlockSpec(
            lambda j, k: (j, 0, k, 0), (None, seq_len_q, None, head_dim)
        ),
        pl.BlockSpec(
            lambda j, k: (j, 0, k, 0), (None, seq_len_k, None, head_dim)
        ),
        pl.BlockSpec(
            lambda j, k: (j, 0, k, 0), (None, seq_len_k, None, head_dim)
        ),
        ...
        pl.BlockSpec(lambda j, k: (j, k, 0), (None, None, seq_len_q)),
        pl.BlockSpec(lambda j, k: (j, k, 0), (None, None, seq_len_q)),
        pl.BlockSpec(lambda j, k: (j, k, 0), (None, None, seq_len_q)),
        pl.BlockSpec(
            lambda j, k: (j, 0, k, 0), (None, seq_len_q, None, head_dim)
    ...
    if segment_ids is None:
      ...
      in_specs.insert(3, pl.BlockSpec(lambda j, k: (j, 0), (None, seq_len_q)))
      in_specs.insert(3, pl.BlockSpec(lambda j, k: (j, 0), (None, seq_len_k)))
    ...
    )(q, k, v, q_segment_ids, kv_segment_ids, out, do_scaled, l, m, delta, dq)
  else:
